...
public class PracticeSessionController : ControllerBase
{
        ...
        [FromQuery] string patientId)
    {
        try
        {
            var result = await _mediator.Send(new GetAttemptCountRequest
            {
                LearnerId = learnerId,
                PatientId = patientId
            });
        ...
        [FromQuery] string patientId)
    {
        try
        {
            var result = await _mediator.Send(new GetActivePracticeSessionRequest
            {
                LearnerId = learnerId,
                PatientId = patientId
            });
        ...
        [FromBody] UpdatePracticeSessionStatusRequest request)
    {
        try
        {
            var result = await _mediator.Send(new UpdatePracticeSessionStatusCommand
            {
                SessionId = id,
